...
#if defined(NDEBUG)
# error "Libertas cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x38f9be92ca89bbe6920ee35b5bd9ac491f3cea6b14b57fab42c15c825c1ae4eb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Libertas: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Libertas Signed Message:\n";
...
{
    // Libertas: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 150 blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Libertas: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Libertas: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1489193709, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "20170310 Outraged Celebrities Apologize ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fbcdd2e5228e3b8f7d1ac3c09bbe72a0740ba1e83d6e41eed904da43fbf3b4a05fcfa55a7ce990356134526b79e6386d8ef99f4ceefe3fb4440ea8a78939433e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1489193709;
        ...
        assert(block.hashMerkleRoot == uint256("0x760bd2cb0c6eb9f0d92bcced2325f481b221ac5c8c878904b49771d2d1736d41"));
...
unsigned char pchMessageStart[4] = { 0x6b, 0xa3, 0xbc, 0xfd }; // Libertas: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LibertasMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LibertasMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LibertasMiner(CWallet *pwallet)
{
    printf("LibertasMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("libertas-miner");
    ...
    try { loop {
        ...
        printf("Running LibertasMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LibertasMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LibertasMiner, pwallet));
}
